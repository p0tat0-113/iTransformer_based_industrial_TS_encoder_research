...
### M4.5. 산출물/집계 스키마 정비 (output_result_recator_plan 기반)
- [x] **op/cmp/agg 분리 정책 확정 및 반영**
  - op_results.json: 단일 평가/진단 결과
  - cmp.json: 두 run 비교 결과
  - agg.json: 다수 run 집계 결과
- [x] Run 메트릭 스키마 정비 (pretrain/downstream 공통)
...
- CMP/AGG 집계 로직 미구현 → M4.5에 체크리스트 반영
