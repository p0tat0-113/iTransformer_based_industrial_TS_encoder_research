...
def main(cfg) -> None:
    ...
    if op_code == "T1":
        ...
        meta_emb_eval = meta_emb[perm]
    elif op_code == "T2":
        if meta_emb is None:
            raise ValueError("T2 requires metadata enabled")
        mask = torch.rand(meta_emb.size(0), device=meta_emb.device) < missing_rate
        meta_emb_eval = meta_emb.clone()
        meta_emb_eval[mask] = 0.0
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if op_code == "S1":
                ...
                x_enc = apply_downsample(x_enc, factor)
            elif op_code == "S3":
                ...
                pass
            elif op_code not in ("S1", "S2", "S3", "R1", "R2", "T1", "T2", "T3"):
                raise ValueError(f"Unknown eval.op_code: {op_code}")

            pred = _predict(model, x_enc, x_mark, meta_emb_eval)
    ...
    results.update(compute_metrics(pred, true))
